...
//        $this->shell = D('shell');
     ...
     * @return string
     */
//    public function getCodeSetKey() {
//        return 'set:product:keyIndex:cid';
//    }
    ...
    public function insert(array $params) {
        ...
        foreach ($params as $row) {
            $row = trim($row);
            if (!$row || $this->redis->exists($this->getNameStringKey($row))) {
                continue;
            }
            $id = $this->redis->incr($this->getIncrementIdStringKey());
            $cid = 'KI' . $id;
            $this->redis->hMSet($this->getHashkey($id), array('id' => $id , 'cid' => $cid, 'name' => $row,
                                                              'addTime' => $time, 'editTime' => $time));
            $this->redis->set($this->getNameStringKey($row), $cid);
            $this->redis->sAdd($this->getStatusSetKey(), $id);
        }
        return true;
    }
    ...
    /**
     * 修改关键指标
     * @param $params 对应修改的字段
     * @return bool
     */
    public function update($params) {
        $id = $params['id'];
        if (!empty($id) && $this->redis->exists($this->getHashkey($id))) {
            $obj = $this->redis->hMGet($this->getHashkey($id), array('cid', 'name'));
            $this->redis->hMSet($this->getHashkey($id), array('name' => $params['name'], 'editTime' => time()));
            if (!empty($params['name']) && $obj['name'] != $params['name']) {
                $this->redis->set($this->getNameStringKey($params['name']), $obj['cid']);
                $this->redis->del($this->getNameStringKey($obj['name']));
            }
            return true;
        } else {
            return false;
