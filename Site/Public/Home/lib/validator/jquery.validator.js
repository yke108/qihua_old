    ...
    Validator.prototype = {
        ...
        _getMsgDOM: function(el, msgOpt) {
            ...
            if (!$msgbox.length) {
                ...
                $msgbox = $('<'+ msgOpt.wrapper + '>');
                $msgbox.attr('class', (CLS_MSG_BOX + (msgOpt.cls ? ' ' + msgOpt.cls : '')));
                $msgbox.attr('style', ( msgOpt.style || ''));
                $msgbox.attr('for', datafor);
...
$.validator.config({
    ...
    rules: {
        ...
        email: [/^[\w\+\-]+(\.[\w\+\-]+)*@[a-z\d\-]+(\.[a-z\d\-]+)*\.([a-z]{2,4})$/i, "Please enter the Email Address in correct format!"],
        url: [/^(https?|s?ftp):\/\/\S+$/i, "请填写有效的网址"],
        qq: [/^[1-9]\d{4,}$/, "请填写有效的QQ号"],
        IDcard: [/^\d{6}(19|2\d)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)?$/, "请填写正确的身份证号码"],
        tel: [/^(?:(?:0\d{2,3}[\- ]?[1-9]\d{6,7})|(?:[48]00[\- ]?[1-9]\d{6}))$/, "请填写有效的电话号码"],
        mobile: [/^1[3-9]\d{9}$/, "Please enter the phone number in correct format!"],
    ...
    messages: {
        0: "该字段",
        fallback: "Please enter the {0}in correct format!",
        // loading: "remote checking...",
        error: "网络异常",
        timeout: "请求超时",
        required: "Please enter a Valid {0}.",
        remote: "{0}已被使用",
        integer: {
            '*': "请填写整数",
            '+': "请填写正整数",
