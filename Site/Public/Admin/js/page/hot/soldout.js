    ...
    var pageGridConfig = {
        ...
        columns: [
            [
                ...
                { field: 'expireTip', title: '有效期截止', align: 'center', width: '6%' },
                ...
                { field: 'state', title: '状态', align: 'center', width: '15%',
                    formatter: function(value, row, index) {
                        ...
                        return '<p style="color:#999">'+row.reason.state+'</p><p>'+row.reason.addTime+'</p><p>【<span style="color:#f00;">'+row.reason.reason+'</span>】</p>';
 ...
 //渲染操作按钮
    function renderOperateBtns(value, row, index) {
        return '<div class="operate-wrap">'
                +'<a href="javascript:void(0);" data-code="'+row.productDepotCode+'"  data-id="' + row.id + '" class="operate-btn js_agree">恢复上架</a>'
                +'</div>';
    }
    ...
    $('.js_batchDel').linkbutton({
        onClick: function() {
            postDataGridMulti('/Admin/Hot/del', $(this), '#dataGrid');
        }
    });
    //审核通过
