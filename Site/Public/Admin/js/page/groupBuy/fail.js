$(function() {
    ...
    var pageGridConfig = {
        ...
        columns: [
            [
                ...
                { field: 'goodName', title: '商品名', align: 'center', width: '15%' },
                { field: 'img', title: '主图', align: 'center', width: '10%', formatter: function(value, row, index){
                    return '<img src="'+row.img+'" style="max-height: 30px;">'
                } },
                { field: 'storeName', title: '店铺名', align: 'center', width: '10%' },
                { field: 'type', title: '分类', align: 'center', width: '10%' }, 
                { field: 'reason', title: '原因', align: 'center', width: '15%' }, 
                { field: 'reviewTime', title: '审核时间', align: 'center', width: '10%' },
    ...
    function renderOperateBtns(value, row, index) {
        return '<div class="operate-wrap">'
                ...
                +'<a href="javascript:void(0);" data-id="' + row.id + '" class="operate-btn js_revokeOne"> 撤销审核</a>'
                +'</div>';
    }
    ...
    //********使用本地数据测试,上线时删除********//
    $('#dataGrid').datagrid('loadData', failData);
    //********使用本地数据测试,上线时删除********//
    //
    ...
    //撤销审核
    $(document).on('click', '.js_revokeOne', function(e){
        alert('撤销审核');
    })
});
