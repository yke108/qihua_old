$(function() {
               ...
               { field: 'productCode', title: '商品编号', align: 'center', width: '8%',formatter:function(value,row,index){
                ...
                { field: 'state', title: '状态', align: 'center', width: '12%',
                    formatter: function(value, row, index) {
                        if(row.state == 3) {
                            return '<p>已撤销</p><p>【<span style="color:#f00;">'+row.reason.reason+'</span>】<br/>'+row.reason.addTime+'</p>';
                        }
                    }
                },
    ...
    function renderOperateBtns(value, row, index) {
        return '<div class="operate-wrap">'
                +'<a href="javascript:void(0);"  data-id="' + row.id + '" class="operate-btn js_agree">恢复通过</a>'
                +'</div>';
    }
    ...
    //         if (r){
    //             window.location.href = '/Admin/Store/expStore?id=3';
    //         }
    //     });
    // });
    //批量导出
    $(document).on('click', '#js_express', function(){
        ...
        $.messager.confirm('确认提示', '您确认要批量导出吗', function(r){
            if (r){
                window.location.href = '/Admin/Store/expStore?id=3&filter='+idArray.join(',');
            }
        });
    });
    //审核通过
     $(document).on('click', '.js_agree', function() {
        var self = $(this),
            ...
            //先重置表单的值
        $.messager.confirm('确认提示', '您确认要恢复通过此商品吗？', function(r){
            if (r){
                var ajaxData = {
                    url: '/Admin/Store/renewStatus',
                    data: {
                        id: id
                    }
                }
    ...
    $('#js_batchDel').linkbutton({
        onClick: function() {
            postDataGridMulti('/Admin/Store/del', $(this), '#dataGrid');
        }
    });
    //分类
