$(function() {
    ...
    $('#js_roleList').on('click', function(){
        var title = $('#js_userRole').val();
        var departmentId = $('#js_department').combotree('getValue');
                    ...
                    for( var j = 0, lv2; lv2 = lv1.children[j++]; ) {
                       html += '<div class="item">'
                            ...
                            +'<div class="check-wrap">'
                                if(lv2.children && lv2.children.length) {
    ...
    $(document).on('click', '#js_express', function(){
        $.messager.confirm('确认提示', '您确认要批量导出吗', function(r){
            if (r) {
                var title = $('#js_userRole').val();
                var departmentId = $('#js_department').combotree('getValue');
                window.location.href = '/Admin/Auth/expAuth?title=' + title + '&departmentId=' + departmentId;
            }
        });
    });
