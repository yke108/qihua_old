$(function() {
    ...
    //初始化表格配置参数
    var pageGridConfig = {
            url: '/Admin/Supply/findGoods', //请求路径
            columns: [
                [
                    { field: 'number', title: '信息编号', align: 'center', width: '15%' }, {
                        ...
                        formatter: function(value, row, index) {
                            return '<a href="javascript:void(0);" data-title="供应详情-' + row.title + '" data-href="/Admin/Supply/details?id=' + row.id + '" data-id="' + row.id + '" class=" js_iframeLink">' + row.title + '</a>'
                        }
                    },
    ...
    $(document).on('click', '#js_express', function(){
        $.messager.confirm('确认提示', '您确认要批量导出吗', function(r){
            if (r){
                window.location.href = '/Admin/BuyOffer/expFind';
            }
        });
    });
});
